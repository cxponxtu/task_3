...
def create_user(conn,username,password):
    ...
    if db_user:
        ...
        db.commit()
        crr.execute(f"CREATE TABLE a_{username} (qid INT(255), PRIMARY KEY (qid))")
        db.commit()
        send(conn,"!$ua")
...
def add_ques(conn,data,ans):
    ...
    id = crr.fetchall()
    crr.execute(f"INSERT INTO a_{user} VALUES ({id[0][0]})")
    db.commit()
    send(conn,"!$qt")
...
def send_ques(conn,data):
    ...
    crr.execute(f"SELECT id,ques,op1,op2,op3,op4 from question WHERE id NOT IN (SELECT qid from a_{data.user}) {sql}")
    ques = crr.fetchall()
    if ques:
        send(conn,"!$qi")
        send(conn,ques[0][1])
    ...
    while True:
        ...
        if data.size == 0:
            try : 
                ...
                conn.close()
                with c_lock: 
                    del conns[data.addrs] 
                print(f"[DISCONNECTED] {data.addrs[0]}:{data.addrs[1]}")
                print(f"[CONNECTIONS] {threading.active_count() - 2}")
                break
            finally:
                pass
        else:
...
try:
    ...
    while True:
        ...
        conn , addrs = lisock.accept()
        conn.setblocking(False)
        print(f"[CONNECTED] {addrs[0]}:{addrs[1]}")
        data = dat(addrs, "", 0, "", 0, 0)
        new_con = threading.Thread(target=service, args=(conn,data))
        with c_lock:
            conns[addrs] = conn
        new_con.start()
    ...
    # Closing all threads by setting shutdown flag
    global shutdown
    shutdown = True
